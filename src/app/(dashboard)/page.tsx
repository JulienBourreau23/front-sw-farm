...
function StatRow({ avg, maxRunes }: { avg: Avg; maxRunes: number }) {
  ...
  const decimals = ["HP_FLAT", "ATK_FLAT", "DEF_FLAT", "SPD"].includes(
    avg.stat_code,
  )
    ? 0
    : 1;
...
export default function DashboardPage() {
  ...
  const { data: top3Data } = useQuery({
    queryKey: ["top3-by-stat", topStat],
    queryFn: () => statsApi.getTop3ByStat(topStat, 5),
    staleTime: 1000 * 60 * 10,
  });

  const globalAverages: Avg[] = globalData?.averages ?? [];
  const displayAverages: Avg[] =
    selectedSetId !== undefined ? (setData?.averages ?? []) : globalAverages;
  const maxRunes = Math.max(...displayAverages.map((a) => a.rune_count), 1);
  const isLoading =
    globalLoading || (selectedSetId !== undefined && setLoading);
  const decimals = (code: string) =>
    ["HP_FLAT", "ATK_FLAT", "DEF_FLAT", "SPD"].includes(code) ? 0 : 1;
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-xl border bg-card p-6 text-center text-muted-foreground">
          ...
          <p className="text-sm mt-1">
            Importe ton fichier JSON SW pour commencer.
          </p>
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          {selectedSetId
            ? `Set ${RUNE_SETS[selectedSetId]}`
            : "Vue globale — tous sets confondus"}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="rounded-xl bg-card border p-4">
          <p className="text-xs text-muted-foreground truncate">
            VIT moy. grind{setLabel}
          </p>
          <p className="text-2xl font-semibold mt-1 text-primary">
            {displayAverages
              .find((a) => a.stat_code === "SPD")
              ?.avg_with_grind.toFixed(1) ?? "—"}
          </p>
        </div>
        <div className="rounded-xl bg-card border p-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-xs text-muted-foreground ml-2 shrink-0">
                            {totalRunes
                              ? ((item.rune_count / totalRunes) * 100).toFixed(
                                  1,
                                )
                              : 0}
                            %
