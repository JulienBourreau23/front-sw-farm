...
export default function RunesPage() {
  ...
  const slot1 = useQuery({ queryKey: ["averages", "slot", selectedSetId, 1, null, userId],        queryFn: () => runesApi.getAverages({ set_id: selectedSetId, slot_no: 1 }),                       enabled: !!userId, staleTime: 1000 * 60 * 5 })
  const slot2 = useQuery({ queryKey: ["averages", "slot", selectedSetId, 2, priStats[2], userId], queryFn: () => runesApi.getAverages({ set_id: selectedSetId, slot_no: 2, pri_stat: priStats[2] }), enabled: !!userId, staleTime: 1000 * 60 * 5 })
  const slot3 = useQuery({ queryKey: ["averages", "slot", selectedSetId, 3, null, userId],        queryFn: () => runesApi.getAverages({ set_id: selectedSetId, slot_no: 3 }),                       enabled: !!userId, staleTime: 1000 * 60 * 5 })
  const slot4 = useQuery({ queryKey: ["averages", "slot", selectedSetId, 4, priStats[4], userId], queryFn: () => runesApi.getAverages({ set_id: selectedSetId, slot_no: 4, pri_stat: priStats[4] }), enabled: !!userId, staleTime: 1000 * 60 * 5 })
  const slot5 = useQuery({ queryKey: ["averages", "slot", selectedSetId, 5, null, userId],        queryFn: () => runesApi.getAverages({ set_id: selectedSetId, slot_no: 5 }),                       enabled: !!userId, staleTime: 1000 * 60 * 5 })
  const slot6 = useQuery({ queryKey: ["averages", "slot", selectedSetId, 6, priStats[6], userId], queryFn: () => runesApi.getAverages({ set_id: selectedSetId, slot_no: 6, pri_stat: priStats[6] }), enabled: !!userId, staleTime: 1000 * 60 * 5 })
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Moyennes par slot et stat principale</p>
        </div>
        <select
